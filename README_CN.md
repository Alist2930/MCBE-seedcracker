...
程序会自动按群系稀有度排序样本，最稀有的群系优先检查，一旦不匹配立即跳过当前种子，大幅提高效率。对每个高32位值，按稀有度顺序依次检查所有群系样本，第一个群系不匹配则立即跳过，避免不必要的计算：
...
> **⚠️ 注意**：ChunkBase 等网站上的群系命名是按照 Java 版的，与基岩版有所不同。例如：Java 版的 `stony_shore` 在基岩版中是 `stone_beach`，Java 版的 `dark_forest` 在基岩版中是 `roofed_forest`。验证时请注意区分。
...
将苍白之园样本改为黑森林 (roofed_forest, ID: 29) 再尝试破解。例如：

```python
# 原样本（1.21.5+ 游戏中是苍白之园）
SAMPLES = [
    (-3695, 5731, 186),   # pale_garden - 在 cubiomes 中可能是 roofed_forest
    ...
]

# 修改后（兼容 cubiomes 1.21.4）
SAMPLES = [
    (-3695, 5731, 29),    # roofed_forest - 尝试用黑森林
    ...
]
```

我们会持续关注 cubiomes 的更新，待其支持 1.21.5+ 后将及时更新本项目。
