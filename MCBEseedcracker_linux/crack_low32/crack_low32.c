...
EXPORT int crack_low32_simple(
    ...
    for (uint64_t w_seed = start; w_seed < end && found_count < max_results; w_seed++) {
        ...
        if (check_mt_seed_linear(r0, ox[0], oz[0], 24)) {
            int all_match = 1;
            
            for (int i = 1; i < num_targets; i++) {
                uint32_t rn = w + r_base[i];
                if (!check_mt_seed_linear(rn, ox[i], oz[i], 24)) {
                    all_match = 0;
                    break;
                }
            }
...
EXPORT int get_num_threads() {
    return 1;
}

EXPORT void set_num_threads(int n) {
}
