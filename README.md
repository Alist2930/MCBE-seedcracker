...
低32位确定后，还确定高32位才能得到完整种子。群系生成依赖于完整的64位种子，通过多层噪声函数计算每个坐标的群系类型。
...
程序会自动按群系稀有度排序样本，最稀有的群系用于 Phase 1 筛选，可大幅提高效率：
...
```
>>> [!] Found seed: -1968172473 (0x-754FEDB9)
```

**这是正常的！** 原因是程序内部使用 `int32_t` 类型存储结果，当值超过 `2,147,483,647` 时会显示为负数。

**解决方法：直接使用负数即可，无需手动转换。**

```python
# crack_high32.py 中直接填负数
LOW32 = -1968172473  # ✅ 正确，程序会自动处理

# 或者用命令行参数
python crack_high32.py --low32 -1968172473
```
...
| 显示值        | 实际值       | 十六进制     |
| ------------- | ------------ | ------------ |
| `-1968172473` | `2326794823` | `0x8AB01247` |

两者位模式完全相同，只是解释方式不同。高32位破解器会自动将有符号数转换为无符号数处理。
...
3. **MC版本设置错误** - 群系生成算法随版本变化
4. **采样高度不当** - 建议 Y >= 150，避免地下群系干扰
5. **群系样本数量不足** - 建议至少 5 个样本
6. **样本选择不当** - 应选择稀有群系（如樱花林、苍白之园），避免常见群系（如平原、海洋）

**解决方法：**

- 确认低32位值正确
- 核对群系样本坐标和ID
- 检查 MC_VERSION_STR 设置
- 提高采样高度
- 选择稀有群系作为样本

### 破解时间过长

**低32位破解：** 正常情况下约 30-40 分钟

**高32位破解：** 正常情况下约 10-20 小时
...
> **注**：稀有度基于地表 Y=150 采样统计。地下群系（dripstone_caves、lush_caves、deep_dark）不参与稀有度排序。
